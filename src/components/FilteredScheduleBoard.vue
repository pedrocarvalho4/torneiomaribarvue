<template>
  <section class="schedule-board">
    ...
    <div class="quick-links">
      <button @click="typeFilter = 'MAS'">{{ t('common.male') }}</button>
      <button @click="typeFilter = 'FEM'">{{ t('common.female') }}</button>
      <button @click="typeFilter = 'MIX'">{{ t('common.mixed') }}</button>
      <button @click="typeFilter = 'JSub16'">{{ t('common.sub16') }}</button>
      <button @click="typeFilter = ''">{{ t('common.all') }}</button>
    </div>
    ...
    <template v-else>
      ...
      <section class="filters-card">
        <h3>{{ t('schedule.viewByField') }}</h3>

        <label>
          {{ t('schedule.selectField') }}
...
const limitedFilteredMatches = computed(() => filteredMatches.value.slice(0, resultLimit))

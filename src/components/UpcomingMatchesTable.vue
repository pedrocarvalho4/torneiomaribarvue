<template>
  <section class="matches-section">
    <div class="matches-header">
      <h2>{{ t('upcomingMatches.title') }}</h2>
      <p>{{ t('upcomingMatches.subtitle') }}</p>
    </div>

    <div v-if="loading" class="state-card">
      {{ t('upcomingMatches.loading') }}
    </div>

    <div v-else-if="error" class="state-card error">
      {{ t('upcomingMatches.loadError') }}
    </div>
    ...
    <div v-else class="table-wrapper">
      <table>
        ...
        <tbody>
          <tr v-for="match in upcomingMatches" :key="`${match.number}-${match.field}-${match.time}`">
            <td>{{ match.number }}</td>
            <td>{{ match.day }}</td>
            <td>{{ match.time }}</td>
            <td>{{ match.field }}</td>
      ...
      <div v-if="upcomingMatches.length === 0" class="state-card">
        {{ t('upcomingMatches.empty') }}
      </div>
    </div>
  </section>
</template>
...
async function fetchSheet(sheet) {
  const url = `https://docs.google.com/spreadsheets/d/${sheetID}/gviz/tq?tqx=out:json&gid=${sheet.gid}&_=${Date.now()}`
...
async function fetchAllSheets() {
  ...
  try {
    const results = await Promise.allSettled(sheets.map((sheet) => fetchSheet(sheet)))
...
const upcomingMatches = computed(() => {
  return rows.value
    .map(({ row, field }) => {
      ...
      const day = row.c[1]?.v || ''
      const time = formatTime(row.c[2])
      const teamA = row.c[4]?.v || ''
      const resultA = row.c[5]?.v?.toString().trim() || ''
      const resultB = row.c[7]?.v?.toString().trim() || ''
      const teamB = row.c[8]?.v || ''
    ...
    .filter((match) => match.field !== 'Campo 21')
...
onMounted(() => {
  fetchAllSheets()
  intervalId = setInterval(fetchAllSheets, 120000)
})
